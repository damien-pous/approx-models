...
Lemma div2_succ_succ n : div2 n.+2 = (div2 n).+1.
Proof.
  have H m : div2 m.+2 = (div2 m).+1 /\ div2 m.+3 = (div2 m.+1).+1
    by elim: m.
  by [].
Qed.
...
Proof.
  rewrite /order div2_succ_succ Nat.even_succ_succ.
  by destruct (even n).
Qed.
...
Section ops.
  ...
  Fixpoint split (p : list C) :=
    match p with
      | [] => ([],[])
      | a::q => let (u,v) := split q in (a::v, u)
    end.

  Fixpoint merge_zero (q : list C) :=
    match q with
      | [] => []
      | b::q' => 0::b::(merge_zero q')
    end.

  Fixpoint merge_rec n (p q : list C) := match n with
    | 0 => []
    | S n' => match p with
        | [] => merge_zero q
        | a::p' => a::(merge_rec n' q p')
      end
  end.

  Definition merge p q := merge_rec (length p + length q) p q.

(*
  Program Fixpoint merge (p q : list C) {measure (length p + length q)} :=
    match p with
      | [] => merge_zero q
      | a::p' => a::(merge q p')
    end.
...
*)
  ...
  Lemma merge_nil_nil : merge [] [] = [].
  Proof. by []. Qed.

  Lemma merge_nil q : merge [] q = merge_zero q.
  Proof. by move: q => [ | b q]. Qed.

  Lemma merge_cons a p q : merge (a::p) q = a::(merge q p).
  Proof. by rewrite /merge /= Nat.add_comm. Qed.

  Lemma merge_split p : merge (split p).1 (split p).2 = p.
  Proof.
  elim: p => [ // | a p /=].
  move: (split p) => [u v] /=.
  by rewrite merge_cons => ->.
  Qed.

End ops.


Lemma eval_split_ n p x : eval_ n p x =
  if (even n) then
    evalCC_ (order n) (split p).1 x + evalSS_ (order n).+1 (split p).2 x
  else
    evalCC_ (order n) (split p).2 x + evalSS_ (order n) (split p).1 x.
Proof.
  elim: p n => [ | a p IHp] n /=.
+ destruct (even n); lra.
+ move: IHp => /(_ n.+1) ->.
...
Lemma eval_split p x : eval p x = evalCC (split p).1 x + evalSS (split p).2 x.
Proof. by apply eval_split_. Qed.

Lemma eval_merge_zero_ n q x : eval_ n (merge_zero q) x =
  if (even n) then
...
Lemma eval_merge_zero q x : eval (merge_zero q) x = evalSS q x.
Proof. apply eval_merge_zero_. Qed.

Lemma eval_merge_rec_ n m p q x : (m = length p + length q)%nat ->
  eval_ n (merge_rec m p q) x =
  if (even n) then
    evalCC_ (order n) p x + evalSS_ (order n).+1 q x
  else
    evalSS_ (order n) p x + evalCC_ (order n) q x.
Proof.
  elim: m p q n => [ | m IHm [|a p] q n H /=].
+ move => [|a p] [|b q] n //=; destruct (even n); lra.
+ rewrite eval_merge_zero_.
  destruct (even n) => /=; lra.
+ move: IHm => /(_ q p n.+1) ->.
* rewrite Nat.even_succ -Nat.negb_even /F /=.
  move: (order_succ n) => [IHe IHo].
...
Lemma eval_merge_ n p q x : eval_ n (merge p q) x =
  if (even n) then
    evalCC_ (order n) p x + evalSS_ (order n).+1 q x
  else
    evalSS_ (order n) p x + evalCC_ (order n) q x.
Proof. by apply eval_merge_rec_. Qed.

Lemma eval_merge p q x : eval (merge p q) x = evalCC p x + evalSS q x.
Proof. apply eval_merge_. Qed.
