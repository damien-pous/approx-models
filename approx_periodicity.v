...
Record Tube C := { pol: list C; rem: C; cont: bool; periodic: bool }.
...
Section n.
 ...
 Definition msingle p: Tube := {| pol := p; rem := 0; cont := true; periodic := false (* Not sure *) |}.

 (** uninformative model  *)
 Definition mbot: Tube := {| pol := 0; rem := bot; cont := false ; periodic := false |}.
 ...
 Definition madd (M N: Tube): Tube :=
   {| pol := pol M + pol N;
      rem := rem M + rem N;
      cont := cont M && cont N;
      periodic := periodic M && periodic N;|}.
 ...
 Definition mmul (M N: Tube): Tube :=
   {| pol := pol M * pol N;
      ...
      cont := cont M && cont N;
      periodic := periodic M && periodic N; |}.
 Definition mzer: Tube := {| pol := 0; rem := 0; cont := true; periodic := true  |}.
 Definition mone: Tube := {| pol := 1; rem := 0; cont := true; periodic := true  |}.
 ...
 (** integration *)
(** Need maybe modifications for periodic functions *)
 ...
 Definition mtruncate (n: nat) (M: Tube): Tube :=
   ...
   {| pol := p; rem := rem M + srange q; cont := cont M ; periodic := periodic M |}.
 ...
 Definition mdiv_aux (f' g' h' w': Tube): E Tube :=
   ...
   | Some mu', Some b' =>
     if is_lt mu' 1 then ret {| pol := pol h';
                                ...
                                cont := cont f' && cont g';
                                periodic := periodic f' && periodic g' |}
   ...
   | Some mu0', Some mu1', Some b' =>
     ...
     if is_lt mu0' 1 then
       if is_lt 0 delta' then
         ...
         if is_lt mu' 1 then ret {| pol := pol h'; rem:=rem h' + sym rmin'; cont := cont f' ; periodic :=  periodic f'|}             
 ...
 Definition mfc (p: list FF): Tube := {| pol := map F2I p; rem := 0; cont := true ; periodic := false |}.
 ...
 Definition mcontains (M: Tube) (f: R -> R) :=
   ...
   /\
   wreflect (is_periodic f) (periodic M)
   /\
 ...
 Lemma mperiod M f: periodic M -> mcontains M f -> is_periodic f.
 Proof. move =>? [ _ b].  move : b => [a _]. by apply (wreflectE a). Qed.
 (** *** basic operations *)
 
 Lemma rmeval_unsafe (M: Tube) f:
   mcontains M f -> forall X x, contains X x -> dom x -> contains (meval_unsafe M X) (f x).
 Proof.
   intros [_ [ _ (p&Hp&H)]] X x Xx HX. rewrite /meval.
